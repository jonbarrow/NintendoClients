...
class SimpleSearchObject(common.Structure):
	...
	def __key(self):
		return (self.id, self.owner, self.attributes, self.metadata, self.community_id, self.community_code, self.datetime, )
	...
	def __eq__(self, other):
		if type(self) is not type(other):
			return NotImplemented
		for field in ['id', 'owner', 'attributes', 'metadata', 'community_id', 'community_code', 'datetime']:
			if getattr(self, field) != getattr(other, field):
				return False
		return True
...
class SimpleSearchDateTimeAttribute(common.Structure):
	...
	def __key(self):
		return (self.unk1, self.unk2, self.unk3, self.unk4, self.start_time, self.end_time, )
...
class SimpleSearchParam(common.Structure):
	...
	def __key(self):
		return (self.unk1, self.unk2, self.conditions, self.unk3, self.range, self.unk4, )
	...
	def __eq__(self, other):
		if type(self) is not type(other):
			return NotImplemented
		for field in ['unk1', 'unk2', 'conditions', 'unk3', 'range', 'unk4']:
			if getattr(self, field) != getattr(other, field):
				return False
		return True
