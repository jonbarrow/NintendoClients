...
class CompetitionRankingScoreData(common.Structure):
	...
	def __key(self):
		return (self.rank, self.pid, self.score, self.last_update, self.team_id, self.metadata, )
	...
	def __eq__(self, other):
		if type(self) is not type(other):
			return NotImplemented
		for field in ['rank', 'pid', 'score', 'last_update', 'team_id', 'metadata']:
			if getattr(self, field) != getattr(other, field):
				return False
		return True
...
class CompetitionRankingScoreInfo(common.Structure):
	...
	def __key(self):
		return (self.season_id, self.scores, self.num_participants, self.team_scores, )
